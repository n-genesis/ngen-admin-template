...
This template features a responsive layout with a dark & light theme, navigation menu on the left, main content. This template's made to be simple, by trying to stick with Bootstrap 5's framwork with little styling and using HTML 5 semantic tags for easy customization. It also doesn't use any JavaScript libraries making it easier add custom operations. It could be a great starter template for small project or for the `Views` or `temapltes` in small custom application using CodeIgniter 4, CakePHP, or Symfony. It looks great on mobile screen all the way up to desktop monitors. Feel free to use it customize it and use it in your next project.  
