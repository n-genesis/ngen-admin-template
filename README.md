...
This template features a responsive layout with: a dark & light theme, navigation menu on the left for mobile devices, & wrapper `main` element to keep the content orginized. Made to be simple by sticking with Bootstrap 5's layout framework. With little custom CSS and instead extending it's styling and using HTML5 semantic tags, makes it easy to customization for your own project. It also doesn't use any JavaScript libraries making it easier add custom operations. It could be a great starter template for small project or for the `Views` or `temapltes` in small custom application using CodeIgniter 4, CakePHP, or Symfony. It looks great on mobile screen all the way up to desktop monitors. Feel free to use it customize it and use it in your next project.  
